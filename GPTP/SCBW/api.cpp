...
#pragma warning(pop)
	...
	int arctangent(int slope) {
		...
		const unsigned int tangentTable[] = {
		    7,   13,  19,  26,  32,  38,   45,   51,   58,   65,   71,   78,   85,   92,   99,    107,
		    114, 122, 129, 137, 146, 154,  163,  172,  181,  190,  200,  211,  221,  233,  244,   256,
		    269, 283, 297, 312, 329, 346,  364,  384,  405,  428,  452,  479,  509,  542,  578,   619,
		    664, 716, 775, 844, 926, 1023, 1141, 1287, 1476, 1726, 2076, 2600, 3471, 5211, 10429, (unsigned int)-1};
	...
	u32 getSuppliesAvailable(u8 playerId, u8 raceId) {
		...
		if (raceId == RaceId::Protoss || raceId == RaceId::Terran || raceId == RaceId::Zerg) {
	...
	bool checkUnitCollisionPos(CUnit *unit, const Point16 *inPos, Point16 *outPos, Box16 *moveArea, bool hideErrorMsg,
	                           u32 someFlag) {
		assert(unit);
		assert(inPos);
		assert(outPos);
