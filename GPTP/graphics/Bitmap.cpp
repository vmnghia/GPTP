...
namespace graphics {
	...
	u8 gbColorTable[] = {0,  0,  0, 1, 2, 3,  4,  5,  8,  0,  0, 0,  0,  0,  9,  10,
	                     11, 12, 0, 0, 0, 13, 14, 15, 16, 17, 0, 18, 19, 20, 21, 23};

	u8 gbFontColors[24][8] = {
	    {0xC0, 0x9B, 0x9A, 0x95, 0x43, 0x00, 0x00, 0x28}, {0x56, 0xA7, 0x6D, 0x65, 0x5C, 0x00, 0x00, 0x8A},
	    {0x41, 0xFF, 0x53, 0x97, 0x47, 0x00, 0x00, 0x8A}, {0x40, 0x96, 0x49, 0x90, 0x42, 0x00, 0x00, 0x8A},
	    {0xA8, 0xAE, 0x17, 0x5E, 0xAA, 0x00, 0x00, 0x8A}, {0xB5, 0x75, 0xBA, 0xB9, 0xB7, 0x00, 0x00, 0x8A},
	    {0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08}, {0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08},
	    {0x8A, 0x6F, 0x17, 0x5E, 0xAA, 0x8A, 0x8A, 0x8A}, {0x28, 0xA5, 0xA2, 0x2D, 0xA0, 0x8A, 0x8A, 0x8A},
	    {0x8A, 0x9F, 0x9E, 0x9D, 0xB7, 0x8A, 0x8A, 0x8A}, {0x8A, 0xA4, 0xA3, 0xA1, 0x0E, 0x8A, 0x8A, 0x8A},
	    {0x8A, 0x9C, 0x1C, 0x1A, 0x13, 0x8A, 0x8A, 0x8A}, {0x8A, 0x13, 0x12, 0x11, 0x57, 0x8A, 0x8A, 0x8A},
	    {0x8A, 0x54, 0x51, 0x4E, 0x4A, 0x8A, 0x8A, 0x8A}, {0x8A, 0x87, 0xA6, 0x81, 0x93, 0x8A, 0x8A, 0x8A},
	    {0xB5, 0xB9, 0xB8, 0xB7, 0xB6, 0x8A, 0x8A, 0x8A}, {0x8A, 0x88, 0x84, 0x81, 0x60, 0x8A, 0x8A, 0x8A},
	    {0x8A, 0x86, 0x72, 0x70, 0x69, 0x8A, 0x8A, 0x8A}, {0x8A, 0x33, 0x7C, 0x7A, 0xA0, 0x8A, 0x8A, 0x8A},
	    {0x8A, 0x4D, 0x26, 0x23, 0x22, 0x8A, 0x8A, 0x8A}, {0x8A, 0x9A, 0x97, 0x95, 0x91, 0x8A, 0x8A, 0x8A},
	    {0x8A, 0x88, 0x84, 0x81, 0x60, 0x8A, 0x8A, 0x8A}, {0x8A, 0x80, 0x34, 0x31, 0x2E, 0x8A, 0x8A, 0x8A}};
	...
	bool Bitmap::blitString(const char *pszStr, int x, int y, u8 size) {
		...
		if (x + fnt->getTextWidth(pszStr) < 0 || y + fnt->getTextHeight(pszStr) < 0 || x >= this->getWidth() ||
		    y >= this->getHeight())
			return false;
		...
		for (int c = 0; pbChars[c]; ++c) {
			...
			// Korean support
			if (GetUserDefaultLangID() == MAKELANGID(LANG_KOREAN, SUBLANG_KOREAN) && IsDBCSLeadByte(pbChars[c]) &&
			...
			if (color != ~0) {
				...
				for (int i = 0, pos = 0; pos < fntChr->getHeight() * fntChr->getWidth(); ++i, ++pos) {
					...
					if (offset >= this->getWidth() * this->getHeight())
						break;
					if (offset < 0)
						continue;
	...
	void Bitmap::drawArrowLine(int x1, int y1, int x2, int y2, ColorId color) {
		// If horizontal
		if (y1 == y2)
			this->drawHorizontalLine(x1, x2, y1, color);
	...
	void Bitmap::drawBox(int left, int top, int right, int bottom, ColorId color) {
		if (left > right)
			std::swap(left, right);
		if (right < 0 || left >= this->getWidth())
			return;

		if (top > bottom)
			std::swap(top, bottom);
		if (bottom < 0 || top >= this->getHeight())
			return;
	...
	void Bitmap::drawArrow(int x, int y, int length, int angle, ColorId color) {
		const double arrowAngle = 3.14159265358979323846264338327950288 / 4;
		double lineAngle = 3.14159265358979323846264338327950288 * angle / 180;
		int arrowX1 = x - lround(length * cos(lineAngle + arrowAngle));
		int arrowY1 = y - lround(length * sin(lineAngle + arrowAngle));
		int arrowX2 = x - lround(length * cos(lineAngle - arrowAngle));
		int arrowY2 = y - lround(length * sin(lineAngle - arrowAngle));

		this->drawLine(x, y, arrowX1, arrowY1, color);
		this->drawLine(x, y, arrowX2, arrowY2, color);
	}
	...
	void Bitmap::drawChevronLine(int x1, int y1, int x2, int y2, ColorId color, int dash, int gap, int direction,
		...
		int count = ((*elapsedTimeFrames + speed) % (dash + gap)) * direction;
		...
		int x = x1, y = y1;
		this->drawDot(x1, y1, color);
		this->drawDot(x2, y2, color);
		if (count % (dash + gap) < dash && !hideDashes)
			this->drawDot(x, y, color);
		count++;
		while (x != x2 || y != y2) {
			const int e2 = err * 2;
			...
			if (count % (dash + gap) < dash && !hideDashes)
				this->drawDot(x, y, color);
			if (count % (dash + gap) == 0 || count % (dash + gap) == dash) {
				this->drawArrow(x, y, arrowLength, lineAngle * 180 / 3.14159265358979323846264338327950288, color);
			}
			count++;
		}
	}
